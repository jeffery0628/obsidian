...
2. 缩小key粒度（增大数据倾斜可能性，降低每个task的数据量）key的数量增加，可能使数据倾斜更严重。
...
![[700 Attachments/Pasted image 20220319174949.png]]
...
![[700 Attachments/Pasted image 20220319175120.png]]

普通的join是会走shuffle过程的，而一旦shuffle，就相当于会将相同key的数据拉取到一个shuffle read task中再进行join，此时就是reduce join。但是如果一个RDD是比较小的，则可以采用==广播==小RDD全量数据+map算子来实现与join同样的效果，也就是map join，此时就不会发生shuffle操作，也就不会发生数据倾斜。
...
![[700 Attachments/Pasted image 20220319175434.png]]
...
![[700 Attachments/Pasted image 20220319175645.png]]
...
将两个处理后的RDD，进行join操作。
![[700 Attachments/Pasted image 20220319175950.png]]
